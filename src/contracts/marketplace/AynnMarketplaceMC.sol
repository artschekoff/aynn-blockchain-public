...
contract AynnMarketplaceMC is ERC2981Marketplace, MultiCurrency, ReentrancyGuard  {
    ...
    nonReentrant
  {
    ...
    emit ItemListed(_nft, _tokenId, msg.sender, address(this), _payToken, _price);
  }
  ...
  // todo: payToken argument is usless here???
  function purchaseItem(
    ...
    if (_payToken == address(0) || _payToken != listedItem.payToken) {
      revert InvalidPayToken();
    }

    uint256 _totalPrice = _getPriceWithRoyalties(_nft, listedItem.tokenId, listedItem.price);

    if (IERC20(_payToken).allowance(msg.sender, address(this)) == 0) {
      revert NotAllowedToPay(_nft, _tokenId);
    }
    ...
    _transferMarketplaceRoyalty(_nft, listedItem.tokenId, _payToken, listedItem.price);

    // transferring contract royalties (ERC2981 standart)
    _transferERC2981Royalties(listedItem.nft, _tokenId, _payToken, listedItem.price);
    ...
    emit ItemPurchased(_nft, _tokenId, listedItem.seller, msg.sender, _payToken, listedItem.price);
  }
  ...
  function updateListing(
    ...
    s_listings[_nft][_tokenId].payToken = _payToken;

    emit ItemListed(_nft, _tokenId, msg.sender, address(this), _payToken, _newPrice);
  }
